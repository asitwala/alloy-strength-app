...
export default {
  ...
  methods: {
    ...
    updateSpecial(buttonInfo) {
      ...
      this.subworkouts.forEach((subworkout, subworkoutIndex) => {
        subworkout.dataTableItems.forEach((row, rowIndex) => {
          row.inputs.forEach((input, inputIndex) => {
            if (
              input &&
              (input.status === "Empty" || input.status === "Filled")
            ) {
              tempKey = input.code;
      ...
      WorkoutService.updateSpecial(userId, vWID, patternNumber, body).then(
        response => {
          if (response) {
            this.fetchWorkoutInfo();
          }
    ...
    getSpecificWorkout() {
      let index = this.workoutDates
        .map(date => date.Date)
        .indexOf(this.selectedWorkoutDate);
      if (index >= 0) {
    ...
    formatWorkoutDates() {
      this.workoutDates.forEach(date => {
        this.formattedWorkoutDates.push(
          `Week ${date.Week}, Day ${date.Day}: ${date.Date}`
        );
      });
    },
    fetchWorkoutInfo() {
      ...
      WorkoutService.fetchWorkoutInfo(UserId, workoutId)
        .then(response => {
  ...
  data() {
    return {
      ...
      noWorkoutMessage:
        "There is no workout scheduled for today. Get some rest!",
